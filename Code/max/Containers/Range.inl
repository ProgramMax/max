...
namespace Containers
{
	...
	template< typename T >
	MAX_SEMI_PURE_DEFINITION( constexpr Range< T >::Range( const T Minimum, const T Maximum ) noexcept )
		: Minimum( Minimum )
		, Maximum( Maximum )
	{
	}
	...
	template< typename T >
	MAX_PURE_DEFINITION( Range< T > MakeRange( const T Value1, const T Value2 ) noexcept )
	{
